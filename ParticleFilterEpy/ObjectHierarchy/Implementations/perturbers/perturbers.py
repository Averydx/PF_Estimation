...
class ParamOnlyMultivariate(Perturb): 
    ...
    '''Uses a custom multivariate normal implementation to optimize this step of the algorithm, see Utils.py for details'''
    def randomly_perturb(self,ctx:Context,particleArray:List[Particle]):
        cov = np.diag([self.hyperparameters['cov'] for _ in range(2)])
